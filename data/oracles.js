...
export const fetchOraclePrices = async () => {
  const prices = []
  const response0 = await fetch('https://api.cfidev.org/v1/oracle/prices/')
  const { data: data0, cursor: cursor0 } = await response0.json()
  prices.push(...data0)
  const response1 = await fetch(
    `https://api.cfidev.org/v1/oracle/prices?cursor=${cursor0}`,
  )
  const { data: data1, cursor: cursor1 } = await response1.json()
  prices.push(...data1)
  const response2 = await fetch(
    `https://api.cfidev.org/v1/oracle/prices?cursor=${cursor1}`,
  )
  const { data: data2, cursor: cursor2 } = await response2.json()
  prices.push(...data2)
  const response3 = await fetch(
    `https://api.cfidev.org/v1/oracle/prices?cursor=${cursor2}`,
  )
  const { data: data3, cursor: cursor3 } = await response3.json()
  prices.push(...data3)
  const response4 = await fetch(
    `https://api.cfidev.org/v1/oracle/prices?cursor=${cursor3}`,
  )
